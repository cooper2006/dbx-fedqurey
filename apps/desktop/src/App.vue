...
import SqlLibraryPanelWrapper from "@/components/layout/SqlLibraryPanelWrapper.vue";
...
const { message: toastMessage, visible: toastVisible, type: toastType, action: toastAction, toast } = useToast();
...
function fixWithAi(errorMessage: string) {
  ...
  nextTick(() => aiPanelRef.value?.triggerAction("fix", errorMessage));
}
...
function analyzeHistoryWithAi(entry: HistoryEntry) {
  ...
  nextTick(() => aiPanelRef.value?.triggerAction("explain", buildHistoryAiAnalysisPrompt(entry)));
}
    ...
    <TooltipProvider :delay-duration="300">
      <div class="h-screen w-screen max-w-full min-w-[760px] min-h-[600px] flex flex-col bg-background text-foreground overflow-hidden" :style="appUiFontFamilyStyle">
        ...
        <div :class="isClassicLayout ? 'app-layout-classic flex-1 flex min-h-0' : 'app-panel-gutter flex-1 flex min-h-0 gap-1 p-1'">
          ...
          <HistoryPanel :open="showHistory" :classic-layout="isClassicLayout" @close="showHistory = false" @restore="restoreHistorySql" @analyze-ai="analyzeHistoryWithAi" />

          <SqlLibraryPanelWrapper :open="showSqlLibraryPanel" :classic-layout="isClassicLayout" @close="toggleSqlLibrary" />
        </div>
      ...
      <Teleport to="body">
        <Transition name="toast">
          <div
            v-if="toastVisible"
            class="fixed bottom-6 inset-x-0 w-max max-w-[90vw] sm:max-w-3xl mx-auto z-99999 px-4 py-3 rounded-lg text-sm shadow-lg select-text whitespace-pre-wrap break-words flex items-center gap-3"
            :class="{
              'bg-green-600 text-white': toastType === 'success',
              'bg-red-600 text-white': toastType === 'error' || toastType === 'critical',
              'bg-amber-500 text-white': toastType === 'warning',
              'bg-foreground text-background': toastType === 'info',
            }"
          >
            {{ toastMessage }}
            <button v-if="toastAction" class="ml-2 px-2 py-0.5 rounded text-xs font-medium bg-white/20 hover:bg-white/30 transition-colors" @click="toastAction.onClick">
              {{ toastAction.label }}
            </button>
          </div>
        </Transition>
