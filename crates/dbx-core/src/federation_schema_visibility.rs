...
impl FederationVisibilityConfig {
    ...
    pub fn get_accessible_schemas(&self, available_schemas: &[&str]) -> Vec<String> {
        available_schemas.iter()
            .filter(|s| self.is_schema_accessible(s))
            .map(|s| s.to_string())
            .collect()
    }
...
impl FederationVisibilityManager {
    ...
    pub fn get_or_create_config(&mut self, connection_id: &str) -> &mut FederationVisibilityConfig {
        self.configs.entry(connection_id.to_string()).or_insert_with(|| {
            FederationVisibilityConfig::new(connection_id)
        })
    }
...
mod tests {
    ...
    fn test_serialization() {
        ...
        let deserialized: FederationVisibilityConfig = 
